...
layui.define('layer', function (exports)
{
    ...
    Class.prototype.initIE = function ()
    {
        var that = this
            , options = that.config
            , iframe = $(
            '<iframe id="' + ELEM_IFRAME + '" class="' + ELEM_IFRAME + '" name="' + ELEM_IFRAME + '" frameborder="0"></iframe>')
    ...
    Class.prototype.upload = function (files, type)
    {
            ...
            , ajaxSend = function ()
            {
                ...
                layui.each(items, function (index, file)
                {
                    ...
                    $.ajax({
                        ...
                        {
                            aborted++;
                            that.msg((lang == 'zh') ? '请求上传接口出现异常' : ((lang == 'en')
                                                                      ? 'An exception occurred in the request upload interface' : ''))
                            // that.msg('请求上传接口出现异常');
                            error(index);
                            allDone();
                        }
            ...
            , iframeSend = function ()
            {
                ...
                Class.timer = setInterval(function ()
                {
                    ...
                    catch (e)
                    {
                        ...
                        that.msg((lang == 'zh') ? '获取上传后的响应信息出现异常' : ((lang == 'en')
                                                                      ? 'The response information obtained after uploading is abnormal'
                                                                      : ''))
                        clearInterval(Class.timer);
                        error();
                    }
            ...
            , done = function (index, res)
            {
                ...
                if (typeof res !== 'object')
                {
                    ...
                    catch (e)
                    {
                        ...
                        return that.msg(
                            (lang == 'zh') ? '请对上传接口返回有效JSON' : ((lang == 'en') ? 'Return valid JSON to the upload interface' : ''))
                    }
                }
        ...
        switch (options.accept)
        {
            case 'file': //一般文件
                if (exts && !RegExp('\\w\\.(' + exts + ')$', 'i').test(escape(value)))
                {
                    ...
                    that.msg(
                        (lang == 'zh') ? '选择的文件中包含不支持的格式' : ((lang == 'en') ? 'The selected file contains unsupported formats'
                                                                            : ''));
            ...
            case 'video': //视频文件
                if (!RegExp('\\w\\.(' + (exts || 'avi|mp4|wma|rmvb|rm|flash|3gp|flv') + ')$', 'i').test(escape(value)))
                {
                    ...
                    that.msg(
                        (lang == 'zh') ? '选择的视频中包含不支持的格式' : ((lang == 'en') ? 'The selected video contains an unsupported format'
                                                                            : ''));
                    return elemFile.value = '';
                }
                break;
            case 'audio': //音频文件
                if (!RegExp('\\w\\.(' + (exts || 'mp3|wav|mid') + ')$', 'i').test(escape(value)))
                {
                    ...
                    that.msg((lang == 'zh') ? '选择的音频中包含不支持的格式' : ((lang == 'en') ? 'The selected audio contains an unsupported' +
                                                                                   ' format' : ''));
                    return elemFile.value = '';
                }
                break;
            default: //图片文件
                ...
                if (check)
                {
                    ...
                    that.msg((lang == 'zh') ? '选择的图片中包含不支持的格式' : ((lang == 'en') ? 'The selected image contains an unsupported' +
                                                                                   ' format' : ''));
                    return elemFile.value = '';
                }
                break;
        ...
        if (options.number && that.fileLength > options.number)
        {
            ...
            return that.msg((lang == 'zh') ? '同时最多只能上传的数量为' + options.number : ((lang == 'en')
                                                                                ? 'The maximum number of uploads allowed at the same time is ' +
                                                                                  options.number
                                                                                : ''))
        }
        //检验文件大小
        if (options.size > 0 && !(device.ie && device.ie < 10))
        {
            ...
            if (limitSize) return that.msg(
                (lang == 'zh') ? '文件不能超过' + limitSize : ((lang == 'en') ? 'File cannot exceed ' + limitSize : ''))
        }
        send();
    };
    //事件处理
        ...
        var that = this
            ...
            , setChooseText = function (files, filename)
            {
                var elemFile = that.elemFile
                    ...
                    // ? files.length + '个文件'
                              ? files.length + ((lang == 'zh') ? '个文件' + limitSize : ((lang == 'en') ? 'files' : ''))
