...
layui.define(['layer-mobile', 'zepto'], function (exports)
{
    ...
    var msgConf = {
        ...
        file: (lang == 'zh') ? '文件' : ((lang == 'en') ? 'file' : '')
        , video: (lang == 'zh') ? '视频' : ((lang == 'en') ? 'video' : '')
        , audio: (lang == 'zh') ? '音频' : ((lang == 'en') ? 'audio' : '')
    };
    ...
    Upload.prototype.init = function ()
    {
        ...
        {
            item = $(item);
            var form = '<form target="' + elemIframe + '" method="' + (options.method || 'post') +
                       '" key="set-mine" enctype="multipart/form-data" action="' + (options.url || '') + '"></form>';
            var type = item.attr('lay-type') || options.type; //获取文件类型
            //包裹ui元素
            if (!options.unwrap)
            {
                form = '<div class="layui-box layui-upload-button">' + form +
                       '<span class="layui-upload-icon"><i class="layui-icon">&#xe608;</i>' + (
                           item.attr('lay-title') || options.title ||
                           ((lang == 'zh') ? ('上传' + (fileType[type] || '图片')) : ((lang == 'en') ? ('upload' +
                                                                                                    (fileType[type] || 'image'))
                                                                                                 : ''))
                       ) + '</span></div>';
            }
    ...
    Upload.prototype.action = function (input, type)
    {
        ...
        switch (type)
        {
            case 'file': //一般文件
                if (ext && !RegExp('\\w\\.(' + ext + ')$', 'i').test(escape(val)))
                {
                    ...
                    layer.msg((lang == 'zh') ? '不支持该文件格式' : ((lang == 'en') ? 'The file format is not supported' : ''), msgConf);
                    return input.value = '';
                }
                break;
            case 'video': //视频文件
                if (!RegExp('\\w\\.(' + (ext || 'avi|mp4|wma|rmvb|rm|flash|3gp|flv') + ')$', 'i').test(escape(val)))
                {
                    ...
                    layer.msg((lang == 'zh') ? '不支持该视频格式' : ((lang == 'en') ? 'The video format is not supported' : ''), msgConf);
                    return input.value = '';
                }
                break;
            case 'audio': //音频文件
                if (!RegExp('\\w\\.(' + (ext || 'mp3|wav|mid') + ')$', 'i').test(escape(val)))
                {
                    ...
                    layer.msg((lang == 'zh') ? '不支持该音频格式' : ((lang == 'en') ? 'The audio format is not supported' : ''), msgConf);
                    return input.value = '';
                }
                break;
            default: //图片文件
                if (!RegExp('\\w\\.(' + (ext || 'jpg|png|gif|bmp|jpeg') + ')$', 'i').test(escape(val)))
                {
                    ...
                    layer.msg((lang == 'zh') ? '不支持该图片格式' : ((lang == 'en') ? 'The image format is not supported' : ''), msgConf);
                    return input.value = '';
                }
                break;
        }
        ...
        var iframe = $('#' + elemIframe), timer = setInterval(function ()
        {
            ...
            catch (e)
            {
                ...
                layer.msg((lang == 'zh') ? '上传接口存在跨域' : ((lang == 'en') ? 'The upload interface exists across domains' : ''), msgConf);
                clearInterval(timer);
            }
            if (res)
            {
                ...
                catch (e)
                {
                    ...
                    return layer.msg((lang == 'zh') ? '请对上传接口返回JSON字符' : ((lang == 'en') ? 'Return the JSON character to the upload interface' : ''),
                        msgConf);
                }
